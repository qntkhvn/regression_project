...
```{r}
space <- read.csv("https://raw.githubusercontent.com/qntkhvn/spacemissions/main/spacemission.csv")
```
...
-   Model $$logit(Pr(Success)) = 2.084 + 0.528 \cdot ActiveYes$$
...
```{r}
rsq <- function(model) {
  1 - summary(model)$deviance / summary(model)$null.deviance
}
```

```{r}
Model <- c("Rocket Status", "Country", "Decade")
Rsquared <- round(c(rsq(mod), rsq(usrus_mod), rsq(dec_mod)), 6)
datatable(rbind(Model, Rsquared), colnames = rep("", 3), options = list(dom = 't'))
```
